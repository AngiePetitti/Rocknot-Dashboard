...
export async function POST(req: NextRequest) {
  ...
  if (!history.some(m => m.role === 'assistant')) {
    return NextResponse.json({ error: 'Ask the analyst at least one question first — the report is built from the conversation.' }, { status: 400 });
  }
  ...
  const system = `You are the in-house data analyst for Rocknot, a music-inspired handbag & accessories brand. Today's date is ${today}. You are turning an analyst chat conversation into a polished, SHAREABLE one-page report.
...
- A small footer: "Generated by the Rocknot AI Analyst · Data through [latest data date]".
